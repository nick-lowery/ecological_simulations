function LV_competition_chaos(L,N,D,P,pillarq,R,dx_mult,dinit,rep)
...
%rng(42)
rng(666) %used for extra set, checking on spurious initial results 
...
if imageq==1
    ...
    file1=['3sp_LV_comp_chaos_L-' num2str(L) ...
	'_N-' num2str(N) ...
        '_D-' num2str(D) ...
        '_P-' num2str(P) ...
...
if dinit > 0
    % random pixels are rewired among replicates
    rng(rep)

    % enforce bounds such that dinit +/- 10% non-pillar pixels are rewired
    rewire = rand(L,L) .* filt_all;
    lwr = floor(sum(filt_all(:)) * dinit * 0.9);
    upr = ceil(sum(filt_all(:)) * dinit * 1.1);
    while or(sum(sum(and(rewire > 0, rewire < dinit))) < lwr, ...
    	     sum(sum(and(rewire > 0, rewire < dinit))) > upr)
        rewire = rand(L,L) .* filt_all;
    end
    ...
    A(and(rewire > 0, rewire < dinit)) = new_A(and(rewire > 0, rewire < dinit));
    B(and(rewire > 0, rewire < dinit)) = new_B(and(rewire > 0, rewire < dinit));
    C(and(rewire > 0, rewire < dinit)) = new_C(and(rewire > 0, rewire < dinit));
end
...
save([file1 '_metadata.mat'], 'L', 'N', 'D', 'P', 'pillarq', 'R', 'dx', 'dt', 'dsc', 'dinit', ...
	'meanA_out', 'meanB_out', 'meanC_out', 'scale_fac_out', 'rep')
