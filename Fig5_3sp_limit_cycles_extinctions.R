...
# 
# The script reads in cleaned metadata and autocorrelation data files, as generated by
...
# supplemental fig: classification process

dat <- acf.data[[443]] %>%
  mutate(t1 = seq(nrow(.))) %>%
  gather(t2, acf, -t1) %>%
  mutate(t2 = as.integer(str_extract(t2, "\\d+"))) 

acfmat <- ggplot(dat, aes(x = t1, y = t2, fill = acf)) + 
  geom_tile() +
  scale_fill_gradientn(colours = pal(7)) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  labs(x = "time (doubling times)", y = "time (doubling times)", fill = "correlation") +
  theme(legend.position = "top", legend.direction = "horizontal", aspect.ratio = 1)

nocyc <- dat %>%
  filter(t1 == 100, t2 >= 100) %>%
  ggplot(aes(x = t2, y = acf)) +
  geom_hline(yintercept = 0.8, col = "red") +
  geom_hline(yintercept = 0, col = "grey") +
  geom_point() +
  labs(y = "autocorrelation", x = "time (doubling times)")

cyc <- dat %>%
  filter(t1 == 300, t2 >= 300) %>%
  ggplot(aes(x = t2, y = acf)) +
  geom_hline(yintercept = 0.8, col = "red") +
  geom_hline(yintercept = 0, col = "grey") +
  geom_point() +
...
traces <- plot_grid(cyc, nocyc, ncol = 1, align = "v")

plot_grid(acfmat, traces, nrow = 1, rel_widths = c(1.4, 1))
